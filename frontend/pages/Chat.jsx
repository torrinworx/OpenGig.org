...
const MessageItem = ({ msg, userIndex }) => {
	...
	const user = users.find(u => u.observer.id.toHex() === msgUserId) ?? null;
...
const CurrentChat = ThemeContext.use(h =>
	AppContext.use(app => (props, cleanup, mounted) => {
		...
		return (
			<div theme="column_fill_contentContainer" style={{ marginTop: 16 }}>
				...
				<div
					...
					style={{
						...
						borderRadius: 8,
						display: 'flex',
						flexDirection: 'column', // oldest -> newest
					}}
				>
...
const Chat = AppContext.use(app =>
	StageContext.use(stage =>
		suspend(Stasis, async (_, cleanup) => {
			...
			const ChatItem = ({ each }) => {
				...
				const participants = (Array.isArray(userIndex) ? userIndex : []).filter(ui =>
					partSet.has(ui.observer.id.toHex())
				);
