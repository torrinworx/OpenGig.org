...
# Key principles of an **Open Source Services**:
1. **Open Source.** The source of an **Open Source Services** are, and always will be in perpetuity, a public resource accessible and modifiable to all. This provides transparency, security, and quality.
2. **Open Statistics.** The hosting, metrics, costs, and expenses of an **Open Source Services** are all publicly visibile, accessible to all, and up to date. Everyone can view how much it costs to run the platform, how many users it hosts, everything. So much so that you can ssh into a machine and view it's metrics safely. This is vitle so that the public understands the true cost of the service, so that they can both gauge it's value, and provide a controlling force inflicted on those running the project to remain truthful and faithfull to their public service.
3. **Open Structure.** The orgnaizational structure of an **Open Source Services** is publicly visible and up to date. Functionally a workers coop, with government-like stickyness to change, communal voting and agreement. The community, contractors and users, are all apart of the company, because they all have a vested interest in its operations, contributinos, and success. Not just the founder, not just the developers, or managers.
4. **Open Cost.** An **Open Source Services** is operated **at cost.** At cost meaning the **Open Statistics** cost to run the service, to serve the public. At cost includes livable (but not excessive) wages to those working on the project, developing it, and contributing their time and knowledge to it. Just as we believe gig workers should be payed a fair wage, developers and those building and managing the **social service** should be payed just as fairly.
...
# TODO:
Things to work on:
- Cite and find sources to back up claims about capitalism. Show examples to better argue for strcutral issues rather than issues with individual companies, show them as symptoms of a larger problem then a direct cause. The gig economy is arguable one fo the worst examples of exploitation, however it was still allowed to propser under this system.

**Weaknesses:**

1. **Tone and Language:** The use of expletives and charged language can detract from the professionalism and perceived credibility. Terms like “Shit Startups” might alienate readers who prefer a more measured discourse.
